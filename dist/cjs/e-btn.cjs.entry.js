...
const EBtn = class {
    constructor(hostRef) {
        ...
        this.renderContent = () => {
            if (this.icon) {
                return (index.h("e-icon", { icon: this.icon }));
            }
            else {
                return [
                    index.h("slot", { name: "prepend-icon" }, this.renderIcon(this.prependIcon)),
                    index.h("slot", null, "Eclipse"),
                    index.h("slot", { name: "append-icon" }, this.renderIcon(this.appendIcon))
                ];
            }
        };
    }
    render() {
        return (index.h("button", { class: this.classes() }, this.renderContent()));
    }
};
EBtn.style = eBtnCss;
