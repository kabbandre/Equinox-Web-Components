# e-expand



<!-- Auto Generated Below -->


## Properties

| Property         | Attribute          | Description | Type     | Default           |
| ---------------- | ------------------ | ----------- | -------- | ----------------- |
| `elevation`      | `elevation`        |             | `string` | `"6"`             |
| `hideButtonIcon` | `hide-button-icon` |             | `string` | `"ph-caret-up"`   |
| `hideButtonText` | `hide-button-text` |             | `string` | `"Show less"`     |
| `maxHeight`      | `max-height`       |             | `string` | `"100px"`         |
| `openButtonIcon` | `open-button-icon` |             | `string` | `"ph-caret-down"` |
| `openButtonText` | `open-button-text` |             | `string` | `"Show more"`     |


## Dependencies

### Depends on

- [e-icon](../e-icon)

### Graph
```mermaid
graph TD;
  e-expand --> e-icon
  style e-expand fill:#f9f,stroke:#333,stroke-width:4px
```

----------------------------------------------

*Built with [StencilJS](https://stenciljs.com/)*
