...
export const findPath = (object, name, value, currentPath: string[] = []): string[] => {
  let matchingPath: string[]
  if (!object || typeof object !== 'object') return
  if (object[name] === value) return currentPath
...
export const setPath = (object, path: string[], value: any) =>
  path.reduce(
    (acc,cur,index) =>
      acc[cur] = path.length === ++index ? value : acc[cur] || {},
    object
  )
